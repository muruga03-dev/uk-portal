...
export const loginFamily = async (req, res) => {
  try {
    ...
    const token = jwt.sign({ id: family._id, role: "family" }, process.env.JWT_SECRET || "secretkey", { expiresIn: "7d" });
...
export const downloadDocument = async (req, res) => {
  try {
    ...
    const doc = family.documents.find(d => d.filename === filename);
    if (!doc) return res.status(404).json({ message: "Document not found in your uploads" });

    const filePath = path.join(__dirname, "../", doc.path);
    if (!fs.existsSync(filePath)) return res.status(404).json({ message: "File does not exist on server" });
